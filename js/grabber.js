...
var gtfoWorkerTaskTimeoutMs = 15000;
...
function gtfoRunWorkerTask(type, payload, transfer) {
	...
	return new Promise((resolve, reject) => {
		var timeout = setTimeout(() => {
			gtfoWorkerRequests.delete(id);
			reject(new Error('Worker task timed out.'));
		}, gtfoWorkerTaskTimeoutMs);

		gtfoWorkerRequests.set(id, {
			resolve: (value) => {
...
function gtfoBuildComments(data) {
	...
	async function renderSource(group, selectedComments, keepScroll, scrollComment) {
		...
		var analysis;
		try {
			analysis = await gtfoAnalyzeSourceInWorker(rawSource, sourceType, prettifySource, selectedComments, scrollComment);
		}
		catch (error) {
			if (activeGroup != group)
				return;

			sourceTitle.textContent = `${group.title} [error]`;
			sourceCode.textContent = `Could not analyze source.\n\n${error.message || error}`;
			return;
		}
